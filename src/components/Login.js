...
function Login() {
    ...
    const handleClickLogin = async (e) => {
        e.preventDefault();
        console.log(await loginUser(mail, password));
    }
            ...
            <div className='flex flex-col justify-center items-center w-full h-full overflow-x-hidden overflow-y-scroll scrollbar-hide rounded-lg bg-white mt-3 shadow-md py-2 px-8 sm:px-3'>
                ...
                <div className='flex flex-wrap gap-6 sm:gap-3 justify-center items-scenter'>
                    ...
                    <form className='flex flex-col gap-4 min-w-[230px] justify-center'>
                        <input value={mail} onChange={(e) => setMail(e.target.value)} type={"text"} className='px-3 py-2 outline-none rounded-lg border border-transparent focus:border-indigo-600 bg-indigo-200 transition-all duration-200' placeholder='Mail'></input>
                        <div className='flex flex-row w-full items-center '>
                            <input value={password} onChange={(e) => setPassword(e.target.value)} type={`${isVisible ? "text" : "password"}`} className='w-full px-3 py-2 outline-none rounded-l-lg border border-transparent focus:border-indigo-600 bg-indigo-200 transition-all duration-200' placeholder='Password'></input>
